...
- **Docker 29 corrupts `bootBuildImage` images** (pods fail to start with containerd `failed to pull and unpack … wrong diff id "sha256:…" calculated on extraction "sha256:…"`): Docker Engine 29's image export mis-references the *uncompressed* base/run-image layers — the buildpack config's `rootfs.diff_ids` and the source run image stay correct, but the exported **manifest** lists the wrong layer blob (e.g. the 3 KB `os-release` layer) in the base-layer slots. containerd validates each layer's content against its declared diffID and refuses the image. The corruption is in the lifecycle creator's manifest assembly running inside the Docker 29 engine, so it reproduces via **both** `docker push` *and* lifecycle publish (`bootBuildImage` with `publish=true`) — cutting another release or switching to publish does **not** fix it. **The real fix is to build with Docker ≤ 28.** Emergency one-off remediation that worked for `0.1.16`: `skopeo copy docker://<image> dir:/tmp/img`, then pull the correct base-layer bytes from the run image (`docker save <run-image>`, gunzip the layers whose uncompressed sha256 matches the config's `diff_ids`), patch the bad manifest slots' `digest`+`size`, re-verify every layer's descriptor digest/diffID, and `skopeo copy dir:/tmp/img docker://<image>` (skopeo authenticates via the docker credential helper). The env-gated `MYFEEDER_PUBLISH` block in `build.gradle.kts` (lifecycle publish) was added during this incident but did **not** avoid the corruption.
